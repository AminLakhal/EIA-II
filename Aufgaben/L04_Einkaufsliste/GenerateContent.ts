  ...
  export function generateContent(){
    for (let i = 0; i < data.initialProduct.length; i++) {
        ...
        let initMenge = String(data.initialProduct[i].quantity);
        let initName = String(data.initialProduct[i].name);
        let initComment = String(data.initialProduct[i].comment);
        let initDate = String(data.initialProduct[i].date);
